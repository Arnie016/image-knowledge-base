...
- `20260525-0211-relief-print-registration-and-brayer-course-plates` at `2026-05-25T02:11:23+08:00` expanded the public corpus into direct-search printmaking education, linocut tool literacy, registration teaching, ink-handling diagnostics, classroom deployment, and proof-of-use bench scenes directly after the film-grammar streak. Primary scale `page`, secondary scale `tabletop`. The most useful learning was that printmaking explainers stay searchable when they lean on exact shop nouns instead of generic art-class language: `medium brayer control`, `hard-edge fidelity`, `registration pins`, `corner tabs`, `U gouge scoop`, `tack strings`, `bench hook`, `V gouges`, `proof wall`, `registration rail`, and `ghost print`. `imagegen/SKILL.md` and `imagegen/references/prompting.md` reinforced one-image-per-prompt discipline and short-label control, `documents/SKILL.md` kept each plate answering one teaching problem, and `iconography-lab/SKILL.md` helped the cream rag paper, ultramarine tabs, oxide-red grease-pencil marks, gum-black ink, scarred birch benches, and steel press parts behave like one internal studio code without drifting into Etsy craft-ad styling. SEO lesson: the strongest results came from pairing one direct query noun with one concrete physical proof object, then adding at least one room or section-format image so the run page did more than stack posters. Anti-repetition note: avoid another immediate return to the same cork-backed cream chart system, blue label tabs, scarred birch bench, or ghost-print proof bench; next pivots should favor `fruit-tree-grafting-and-pruning-decision-sheets`, `lighting-motivation-and-practical-lamp-guides`, `fermentation-vessel-and-spoilage-visual-guides`, or a later printmaking return through block-inking cleanup, paper dampening, or multi-block registration instead of repeating the same starter kit.

- `20260522-0220-cinematography-shot-size-and-blocking-visual-guides` at `2026-05-22T02:20:09+08:00` expanded the public corpus into direct-search film grammar, indie rehearsal planning, storyboard teaching, apartment-scene blocking, and dialogue-scene proof directly after the predictive-history, beekeeping, aquarium, and braiding explainer streak. Primary scale `page`, secondary scale `room`. The most useful learning was that filmmaking explainers stay searchable when they lean on concrete production nouns instead of abstract cinema theory language: `180 degree rule`, `line of action`, `camera blocking floor plan`, `shot reverse shot`, `viewfinder`, `apple box`, `blue spike tape`, `storyboard scene 12`, `rehearsal wall`, `over the shoulder medium shot`, and `apartment dialogue blocking example`. `imagegen/SKILL.md` and `imagegen/references/prompting.md` reinforced one-image-per-prompt discipline, direct-search labels, and restraint against glossy festival-poster drift, `documents/SKILL.md` kept each image solving one teaching problem, and `trailer-director/SKILL.md` pushed the set to include both planning boards and one cinematic proof still instead of staying flat. SEO lesson: one continuity world reused across diagrams, object atlases, storyboard pages, room deployments, and proof frames produces richer per-image search text than isolated film-school posters. Anti-repetition note: avoid another immediate return to the same square dinner table, cork-wall classroom, blue spike tape, apple-box stack, tungsten window argument, or `scene 12` apartment kit; next pivots should favor `relief-print-registration-and-brayer-course-plates`, `fruit-tree-grafting-and-pruning-decision-sheets`, `lighting-motivation-and-practical-lamp-guides`, or `stunt-previs-beat-sheets`.

- `20260521-1956-predictive-history-pressure-map-boards` at `2026-05-21T19:56:51+08:00` expanded the public corpus into documentary analysis, speculative-history teaching, geopolitical board language, war-room installation views, and cinematic editorial proof directly after the beekeeping, aquarium, and protective-braiding explainer streak. Primary scale `page`, secondary scale `world`. The most useful learning was that predictive-history boards stay searchable when they lean on concrete analysis nouns instead of abstract geopolitics rhetoric: `Gibraltar gate`, `Hormuz Strait`, `pipeline corridors`, `alliance cohesion blocks`, `power transition index gauge`, `flashpoint markers`, `projector glow`, `rolled map tubes`, `war-room evidence wall`, and `city-dome window view`. `imagegen/SKILL.md` reinforced one-image-per-prompt generation and preserving built-in output provenance, `documents/SKILL.md` kept each board solving one reader job instead of collapsing into essay text, `presentations/SKILL.md` pushed contact-sheet rhythm so the batch did not flatten into ten similar dark boards, `trailer-director/SKILL.md` pushed thesis-to-proof sequencing and demanded installation plus cinematic proof images, and `iconography-lab/SKILL.md` helped charcoal dossier stock, navy map wash, amber dots, rust grease-pencil marks, brass clips, and acetate overlays behave like one internal analytical code. SEO lesson: severe documentary lanes can still answer direct search queries if titles and hooks stay concrete and format-first. Anti-repetition note: avoid another immediate return to `fragmented century` headlines, charcoal world-order boards, navy map wash, amber status dots, or archive-room projector setups; next pivots should favor `orchid-potting-and-root-health-visual-guides`, `coastal-knot-and-rigging-course-plates`, `legitimacy-propaganda-comparison-boards`, or `refugee-route-and-border-infrastructure-atlases`.

- `20260521-1653-beekeeping-brood-and-hive-inspection-posters` at `2026-05-21T16:53:00+08:00` expanded the public corpus into backyard apiary training, brood-frame literacy, swarm-warning inspection, mite diagnostics, orchard-edge field setup, protective gear, and beekeeper recordkeeping directly after the aquarium science and protective-braiding explainer runs. Primary scale `page`, secondary scale `object`. The most useful learning was that beekeeping explainers stay concrete and searchable when they lean on apiary nouns instead of generic farm language: `patchy brood pattern`, `pollen band`, `open larvae crescent`, `queen cup`, `swarm cell`, `hive tool`, `frame grip`, `queen excluder`, `sticky board tray`, `smoking bee smoker`, `white veil mesh`, and `hive inspection logbook`. `imagegen/SKILL.md` and `imagegen/references/prompting.md` reinforced query-first prompts with short visible labels, `documents/SKILL.md` kept each image answering one inspection problem, `presentations/SKILL.md` kept the batch varied enough to survive contact-sheet view, and `iconography-lab/SKILL.md` helped smoker metal, wax comb, pine supers, burlap fuel, veil mesh, and pollen-yellow markers behave like one internal training code without drifting into honey-brand packaging. SEO lesson: direct query nouns paired with one field-deployment photo and one technical cutaway produce cleaner run-page hooks than ten flat posters alone. Anti-repetition note: avoid another immediate beekeeping rerun with the same orchard blossom rows, white hive bodies, cream training boards, galvanized smoker plume, or paper-logbook spread; next pivots should favor `relief-print-registration-and-brayer-course-plates`, `fruit-tree-grafting-and-pruning-decision-sheets`, `aquarium-quarantine-and-disease-symptom-guides`, or a later apiary return through honey harvest or winter feeding rather than brood basics.

- `20260521-1553-aquarium-nitrogen-cycle-starter-posters` at `2026-05-21T15:53:14+08:00` expanded the corpus into freshwater aquarium science, pet-store onboarding, home tank setup, hobbyist paper tools, and biology-teaching diagrams directly after the protective-braiding classroom branch. Primary scale `page`, secondary scale `object`. The most useful learning was that aquarium explainers stay concrete and searchable when they lean on cycle nouns instead of generic chemistry language: `ammonia test vial`, `nitrite peak`, `nitrate rise`, `filter sponge colony`, `ceramic bio-rings`, `gravel biofilm`, `hang-on-back filter chamber`, `new tank syndrome`, `test-kit tray`, `water-change bucket`, `pegboard training wall`, and `cycle logbook`. `imagegen/SKILL.md` and `imagegen/references/prompting.md` reinforced labeled query-first prompts with distinct one-image calls, `documents/SKILL.md` kept each page solving one teaching problem rather than turning into a crowded worksheet, and `life-science-research/ncbi-pmc-skill/SKILL.md` pushed the wording toward targeted biological nouns instead of vague science talk. Avoid next time: another immediate aquarium-cycle rerun with the same cool white poster boards, black filter hardware, blue bucket, planted tetra starter tank, or pet-store pegboard wall. Strong follow-up seeds should pivot toward `beekeeping-brood-and-hive-inspection-posters`, `relief-print-registration-and-brayer-course-plates`, `freshwater-aquascape-pruning-reference-plates`, or a later aquarium return through disease symptoms, substrate bacteria, or quarantine workflow instead of cycling basics.
